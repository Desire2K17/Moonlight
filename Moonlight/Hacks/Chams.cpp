...
bool Chams::render(void* ctx, void* state, const ModelRenderInfo& info, matrix3x4* customBoneToWorld) noexcept
{
    ...
    if (std::string_view{ info.model->name }.starts_with("models/weapons/v_")) {
        ...
        if (std::strstr(info.model->name + 17, "sleeve"))
            ...
            renderHands();
        else if (!std::strstr(info.model->name + 17, "knife"))
            renderWeapons();
...
void Chams::renderKnife() noexcept
{
    if (!localPlayer || !localPlayer->isAlive())
        return;

    applyChams(config->chams[KNIFE].materials, false, localPlayer->health());
}
...
void Chams::renderPlayer(Entity* player) noexcept
{
    ...
    if (player == localPlayer.get()) {
        ...
        if (config->backtrack.enabled) 
        {
            ...
            if (config->backtrack.allticks)
            {
                for (int x = 0; x < record->size(); x++) {
                    if (record && record->size() && Backtrack::valid(record->front().simulationTime)) {
                        if (!appliedChams)
                            hooks->modelRender.callOriginal<void, 21>(ctx, state, info, customBoneToWorld);
                        applyChams(config->chams[BACKTRACK_OCCLUDED].materials, true, health, record->at(x).matrix);
                        applyChams(config->chams[BACKTRACK].materials, false, health, record->at(x).matrix);
                        interfaces->studioRender->forcedMaterialOverride(nullptr);
                    }
                }
            }
            else
            {
                if (record && record->size() && Backtrack::valid(record->front().simulationTime)) {
                    ...
                    applyChams(config->chams[BACKTRACK_OCCLUDED].materials, true, health, record->back().matrix);
        ...
        //override original after backtrack to prevent homosexuality
