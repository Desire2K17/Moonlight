...
void nadePrediction::draw() noexcept
{
	...
	if (config->misc.nadePredictOnHold && !localPlayer->getActiveWeapon()->pinPulled())
	...
	// draw end nade path
	if (config->misc.nadeend.enabled)
	{
		for (auto& point : endPoints)
			drawList->AddLine(ImVec2(point.first.x, point.first.y), ImVec2(point.second.x, point.second.y), ImColor(config->misc.nadeend.color), 2.f);
	}
	else
	{
		for (auto& point : endPoints)
	...
	//	draw nade path
	if (config->misc.nadeline.enabled)
	{
		for (auto& point : savedPoints)
			drawList->AddLine(ImVec2(point.first.x, point.first.y), ImVec2(point.second.x, point.second.y), ImColor(config->misc.nadeline.color), 1.5f);
	}
	else
	{
		for (auto& point : savedPoints)
