...
static LRESULT __stdcall wndProc(HWND window, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
    static const auto once = [](HWND window) noexcept {
        ...
        config = std::make_unique<Config>("moonlight");
      ...
      //  gui->open = false;  //uncomment this if you want the menu to stay closed when you inject -desire
...
static bool __stdcall createMove(float inputSampleTime, UserCmd* cmd) noexcept
{
    ...
    Misc::edgebug(cmd);
    cmd->viewangles.y -= angle; //boy wut da hell boy fix


    Legitbot::DoStandAloneRecoil(cmd);
    Legitbot::run(cmd);
...
#ifdef _WIN32
__declspec(naked) void __fastcall createMove_Start(void* thisptr, float inputSampleTime, UserCmd* cmd) noexcept
{
    __asm
    {
        mov sendPacket, bl
...
static int __stdcall doPostScreenEffects(int param) noexcept
{
    ...
    if (hooks->fixshitnightmode && interfaces->engine->isInGame())
        Visuals::nightMode();
    else
        Visuals::nightMode();
...
static void __stdcall paintTraverse(unsigned int panel, bool forceRepaint, bool allowForce) noexcept
{
    ...
    if (!alreadyCalled)
    {
        hooks->tahomaBoldAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->tahomaBoldAA, "Tahoma", 12, 500, 0, 0, fontflag_outline, 0, 0);
        hooks->smallFonts = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->smallFonts, "Small Fonts", 9, 500, 0, 0, fontflag_outline, 0, 0);
        hooks->tahomaAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->tahomaAA, "Tahoma", 12, 400, 0, 0, fontflag_antialias | fontflag_dropshadow, 0, 0);
        hooks->tahomaNoShadowAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->tahomaNoShadowAA, "Tahoma", 12, 400, 0, 0, fontflag_antialias, 0, 0);
        hooks->verdanaExtraBoldAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->verdanaExtraBoldAA, "Verdana", 25, 800, 0, 0, fontflag_outline, 0, 0);
...
static void __stdcall frameStageNotify(FrameStage stage) noexcept
{
    ...
    if (stage == FrameStage::RENDER_START) {
        ...
        //agent changer fix!
        if (interfaces->engine->isConnected() && interfaces->engine->isInGame() && localPlayer)
    ...
    if (interfaces->engine->isInGame()) {
        ...
        static ConVar* blueshit = interfaces->cvar->findVar("cl_csm_world_shadows");
        blueshit->setValue(0);
        static ConVar* blueshit2 = interfaces->cvar->findVar("cl_csm_static_prop_shadows");
        blueshit2->setValue(0);
...
{
    //see here: https://i.imgur.com/hcg0NB5.png 
    if (std::uintptr_t(_ReturnAddress()) == memory->listLeaves && localPlayer && interfaces->engine->isConnected() && interfaces->engine->isInGame()) {//this would crash before!

        //we dont want to use autos here!
        RenderableInfo_t* info = *reinterpret_cast<RenderableInfo_t**>(std::uintptr_t(_AddressOfReturnAddress()) + 0x14);
        if (info && info->m_pRenderable) {
            //or here
            Entity* ent = VirtualMethod::call<Entity*, 7>(info->m_pRenderable - 4);
            ...
            //extend world space bounds.
            constexpr Vector min{ -16384.0f, -16384.0f, -16384.0f };
            constexpr Vector max{ 16384.0f, 16384.0f, 16384.0f };
...
static int __fastcall dispatchSound(SoundInfo& soundInfo) noexcept
{
    if (const char* soundName = interfaces->soundEmitter->getSoundName(soundInfo.soundIndex)) {
        ...
        else if (!strcmp(soundName, "land"))
            modulateVolume([](int index) { return config->sound.players[index].landVolume; });
...
void WriteUsercmd(void* buf, UserCmd* a, UserCmd* b) 
{
...
//studiorender
static char __fastcall fuckyou(void* ecx, void* edx, const char* modulename)
{
    return 1;
}

#define STANDARD_BLENDING_RULES void(__thiscall*)(void*, StudioHdr*, Vector*, void*, float, int)
#define UPDATE_CLIENT_SIDE_ANIMATION void(__thiscall*)(void*)

static void __fastcall doExtraBoneProcessing(Entity* entity, uint32_t edx, StudioHdr* hdr, Vector* pos, void* q, matrix3x4* boneMatrix, void* boneList, void* context) noexcept
{
    return;
}
...
static void __fastcall standardBlendingRules(Entity* entity, uint32_t edx, StudioHdr* hdr, Vector* pos, void* q, float currentTime, int boneMask) noexcept
{
    *entity->getEffects() |= 0x8;
    reinterpret_cast<STANDARD_BLENDING_RULES>(hooks->oStandardBlendingRules)(entity, hdr, pos, q, currentTime, boneMask);
    *entity->getEffects() &= ~0x8;
}

static bool __fastcall shouldSkipAnimFrame(Entity* entity, uint32_t edx) noexcept
{
    return false;
}
*/
...
int __stdcall hkGetModuleHandleExA(DWORD dwFlags, LPCSTR lpModuleName, HMODULE* phModule) {

    uint8_t* dllMemoryStart = (uint8_t*)hooks->module;
    const IMAGE_DOS_HEADER* dllDosHeader = (IMAGE_DOS_HEADER*)dllMemoryStart;
    const IMAGE_NT_HEADERS* dllNtHeaders = (IMAGE_NT_HEADERS*)(dllMemoryStart + dllDosHeader->e_lfanew);
    const DWORD dllSize = dllNtHeaders->OptionalHeader.SizeOfImage;
    const uint8_t* dllMemoryEnd = dllMemoryStart + dllSize;
    
    if (dwFlags == GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS && (uint8_t*)lpModuleName >= dllMemoryStart && (uint8_t*)lpModuleName <= dllMemoryEnd) {
        *phModule = GetModuleHandleA("client.dll");
        return TRUE;
    }
...
void Hooks::install() noexcept
{
  ...
  //  bspQuery.init(interfaces->engine->getBSPTreeQuery());
    ...
    //client.hookAt(24, WriteUsercmdDeltaToBuffer);//not sure why we hook this but...
    client.hookAt(37, frameStageNotify);
    clientMode.hookAt(17, shouldDrawFog);
    clientMode.hookAt(18, overrideView);
#ifdef WIN32
    clientMode.hookAt(24, &createMove_Start);
#else
   ...
   // MH_CreateHook(reinterpret_cast<LPVOID>(memory->findPattern(L"filesystem_stdio", "\x8B\x81????\xC3\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\x55\x8B\xEC\x53")), &canLoadThirdPartyFiles, NULL);
  //  MH_CreateHook(reinterpret_cast<LPVOID>(memory->findPattern(L"engine", "\x55\x8B\xEC\x81\xEC????\x53\x8B\xD9\x89\x5D\xF8")), &getUnverifiedFileHashes, NULL);

    //Dont readd, this is fucked and -reps ur trustfactor
 //   MH_CreateHook((void*)(memory->findPattern(L"client", "\x55\x8B\xEC\x56\x8B\xF1\x33\xC0\x57\x8B\x7D\x08\x8B\x8E????\x85\xC9\x7E\x12\x3B\x3C\x86\x72\x06\x3B\x7C\x86\x04\x72\x49\x83\xC0\x02\x3B\xC1\x7C\xEE")), &fuckyou, NULL);
 //   MH_CreateHook((void*)(memory->findPattern(L"engine", "\x55\x8B\xEC\x56\x8B\xF1\x33\xC0\x57\x8B\x7D\x08\x8B\x8E????\x85\xC9\x7E\x12\x3B\x3C\x86\x72\x06\x3B\x7C\x86\x04\x72\x49\x83\xC0\x02\x3B\xC1\x7C\xEE")), &fuckyou, NULL);
 //   MH_CreateHook((void*)(memory->findPattern(L"studiorender", "\x55\x8B\xEC\x56\x8B\xF1\x33\xC0\x57\x8B\x7D\x08\x8B\x8E????\x85\xC9\x7E\x12\x3B\x3C\x86\x72\x06\x3B\x7C\x86\x04\x72\x49\x83\xC0\x02\x3B\xC1\x7C\xEE")), &fuckyou, NULL);
 //   MH_CreateHook((void*)(memory->findPattern(L"materialsystem", "\x55\x8B\xEC\x56\x8B\xF1\x33\xC0\x57\x8B\x7D\x08\x8B\x8E????\x85\xC9\x7E\x12\x3B\x3C\x86\x72\x06\x3B\x7C\x86\x04\x72\x49\x83\xC0\x02\x3B\xC1\x7C\xEE")), &fuckyou, NULL);

    MH_CreateHook((*reinterpret_cast<void***>(GetModuleHandleExA)), hkGetModuleHandleExA, NULL);
//    MH_CreateHook(GetModuleHandleExA, &hkGetModuleHandleExA, NULL);

    MH_CreateHook((LPVOID)memory->findPattern(L"client", "\x55\x8B\xEC\x83\xE4\xF8\x81\xEC????\x53\x56\x8B\xF1\x57\x89\x74\x24\x1C\x80"), doExtraBoneProcessing, NULL);
  //  MH_CreateHook((LPVOID)memory->findPattern(L"client", "\x55\x8B\xEC\x83\xE4\xF0\xB8\xF8\x10"), standardBlendingRules, reinterpret_cast<LPVOID*>(&oStandardBlendingRules));
  //  MH_CreateHook((LPVOID)memory->findPattern(L"client", "\x57\x8B\xF9\x8B\x07\x8B\x80????\xFF\xD0\x84\xC0\x75\x02"), shouldSkipAnimFrame, NULL);

   // MH_CreateHook((void*)(memory->findPattern(L"server", "\x55\x8B\xEC\x56\x8B\xF1\x33\xC0\x57\x8B\x7D\x08\x8B\x8E????\x85\xC9\x7E\x12\x3B\x3C\x86\x72\x06\x3B\x7C\x86\x04\x72\x49\x83\xC0\x02\x3B\xC1\x7C\xEE")), &fuckyou, NULL);
    //balls
