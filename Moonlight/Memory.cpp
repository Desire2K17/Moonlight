...
Memory::Memory() noexcept
{
    ...
    traceToExit = reinterpret_cast<decltype(traceToExit)>(findPattern(L"client", "\x55\x8B\xEC\x83\xEC\x4C\xF3\x0F\x10\x75"));
    ...
    //custom
    renderBeams = *reinterpret_cast<ViewRenderBeams**>(findPattern(L"client", "\xB9????\x0F\x11\x44\x24?\xC7\x44\x24?????\xF3\x0F\x10\x84\x24" + 1));
    ...
    ResetState = findPattern(L"client", "\x56\x6A\x01\x68????\x8B\xF1");
    InvalidateBoneCache = findPattern(L"client", "\x80\x3D?????\x74\x16\xA1????\x48\xC7\x81");
    //isLoadOutAvailable = *(reinterpret_cast<void**>(findPattern(L"client", "\x75\x04\xB0\x01\x5F") - 2));
    SetupVelocityAddress = *(reinterpret_cast<void**>(findPattern(L"client", "\x84\xC0\x75\x38\x8B\x0D????\x8B\x01\x8B\x80")));
    AccumulateLayersAddress = *(reinterpret_cast<void**>(findPattern(L"client", "\x84\xC0\x75\x0D\xF6\x87")));
    ...
    chokeLimit = findPattern(L"engine", "\xB8????\x3B\xF0\x0F\x4F\xF0\x89\x5D\xFC") + 1;
    clientState = **reinterpret_cast<ClientState***>(findPattern(L"engine", "\xA1????\x8B\x80????\xC3") + 1);
    clearDeathNotices = (void(__thiscall*)(DWORD))findPattern(L"client", "\x55\x8B\xEC\x83\xEC\x0C\x53\x56\x8B\x71\x58");
    localPlayer.init(*reinterpret_cast<Entity***>(findPattern(L"client", "\xA1????\x89\x45\xBC\x85\xC0") + 1));
    eyePositionAndVectors = findPattern(L"client", "\x8B\x55\x0C\x8B\xC8\xE8????\x83\xC4\x08\x5E\x8B\xE5");
    fileSystem = **reinterpret_cast<void***>(findPattern(L"engine", "\x8B\x0D????\x83\xC1\x04\x8B\x01\xFF\x37\xFF\x50\x1C\x89\x47\x10") + 2);
}
