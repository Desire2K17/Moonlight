...
void Animations::fake() noexcept
{
    ...
    if (data.sendPacket)
    {
        ...
        *(uint32_t*)((uintptr_t)localPlayer.get() + 0xA68) = 0;
        ...
        data.fakeAngle = fakeanimstate->GoalFeetYaw;
...
void Animations::real() noexcept
{
    ...
    static auto extrapolation = interfaces->cvar->findVar("cl_extrapolate");
    if(jigglebones->getInt() != 0)
        jigglebones->setValue(0);
    if (extrapolation->getInt() != 0)
        extrapolation->setValue(0);
    if (!localPlayer)
        return;
    ...
    if (oldTick != memory->globalVars->tickCount)
    {
        ...
        memory->setAbsOrigin(localPlayer.get(), localPlayer->origin());
        *localPlayer.get()->getEffects() &= ~0x1000;
        *localPlayer.get()->getAbsVelocity() = localPlayer.get()->velocity();
        std::memcpy(&networked_layers, localPlayer.get()->animOverlays(), sizeof(AnimationLayer) * localPlayer->getAnimationLayerCount());
        localPlayer.get()->ClientSideAnimation() = true;
...
void Animations::players() noexcept
{
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); i++)
    {
        ...
        if (!data.player[i].once)
        {
            ...
            data.player[i].abs = entity->getAnimstate()->GoalFeetYaw;
        ...
        while (entity->getAnimstate()->LastClientSideAnimationUpdateFramecount == memory->globalVars->framecount)
            entity->getAnimstate()->LastClientSideAnimationUpdateFramecount -= 1;
        ...
        *entity->getAbsVelocity() = entity->velocity();
        ...
        entity->getAnimstate()->FeetYawRate = 0.f;
...
void Animations::setupResolver(Entity* entity, Resolver::Tick record) noexcept
{
