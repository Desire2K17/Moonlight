...
/*
Read me!! - Blanket
Move this stuff to whereever most of ur math is.
i added some shit to ur vec struct too if u wanna look over that
*/

//Move this to whereever ur math is
float flRandom(float flMin, float flMax) {
...
void RCS2(UserCmd* cmd) {
    if (!interfaces->engine->isConnected() || !interfaces->engine->isInGame())
        return;

    if (!localPlayer || localPlayer->isDefusing())
        return;
    ...
    if (!activeWeapon || !activeWeapon->clip() || activeWeapon->isPistol() || activeWeapon->isSniperRifle() || activeWeapon->isShotgun())
        return;
    ...
    if (!config->legitbot[weaponIndex].standaloneRcs)
        return;

    static Vector OldAimPunch = Vector(0, 0, 0);
    Vector newPunch = (localPlayer->aimPunchAngle() * 2.f);
    Vector FinalAng = cmd->viewangles - (newPunch - OldAimPunch);
    Vector Delta = (cmd->viewangles - FinalAng);

    //Control Ammount Randomization
    float flMultBy = flRandom(0.9, 1.1);
    Delta *= flMultBy;

    if (cmd->buttons & cmd->IN_ATTACK) {
        OldAimPunch = newPunch;

        Delta.Clamp();//how/when to clamp angles (:
        Delta.normalize();
    ...
    newPunch = Vector(0, 0, 0);
    OldAimPunch = Vector(0, 0, 0);
}
//this is a bit poorly optimized as i dont check if were in attack before controlling
void Legitbot::DoStandAloneRecoil(UserCmd* cmd) noexcept {
    RCS2(cmd);//redid this cuz u guys didint like it
...
void Legitbot::run(UserCmd* cmd) noexcept
{
    ...
    if (config->legitbot[weaponIndex].enabled) {
        ...
        //again check this after rcs
        if (!(cmd->buttons & UserCmd::IN_ATTACK) && !(config->legitbot[weaponIndex].aimlock))
            return;
        ...
        Vector vPunch2 = localPlayer->aimPunchAngle();
        vPunch2 *= 2;

        const auto aimPunch = activeWeapon->requiresRecoilControl() ? vPunch2 : Vector(0, 0, 0);
        ...
        for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
            ...
            for (int j = 0; j < 19; j++)
            {
                ...
                {
                    if (config->legitbot[weaponIndex].visibleOnly && !entity->isVisible(bonePosition))
                        continue;
        ...
        if (bestTarget && (config->legitbot[weaponIndex].ignoreSmoke || !memory->lineGoesThroughSmoke(localPlayer->getEyePosition(), bestTarget, 1))) {
            ...
            if (memory->globalVars->serverTime() - lastTime >= config->legitbot[weaponIndex].reactionTime / 1000.0f)
            {
                ...
                if (!config->legitbot[weaponIndex].silentFov)
                    interfaces->engine->setViewAngles(cmd->viewangles);
            }
