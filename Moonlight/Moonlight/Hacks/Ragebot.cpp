...
static float canScan(Entity* entity, const Vector& destination, const WeaponInfo* weaponData, int minDamage, bool allowFriendlyFire) noexcept
{
    if (!localPlayer)
        return -1;
...
void Ragebot::run(UserCmd* cmd) noexcept
{
    ...
    if (config->ragebot[weaponIndex].enabled && (cmd->buttons & UserCmd::IN_ATTACK || config->ragebot[weaponIndex].autoShot)) {
        ...
        auto ap1 = localPlayer->aimPunchAngle();
        ap1 *= 2;//kinda gay code here lol. sorry ):
        const auto aimPunch = ap1;
        ...
        for (const auto& target : enemies) {
          ...
          //  Animations::finishSetup(entity);
            for (int i = 0; i < 19; i++)
            {
                ...
                matrix3x4 pBoneMatrix[256];
                interfaces->entityList->getEntity(target.id)->setupBones(pBoneMatrix, 256, 0x7FF00, memory->globalVars->currenttime);


                for (auto bonePosition : Multipoint(entity, pBoneMatrix, i, weaponIndex, firstScan))
                {
                    firstScan = false;
                    if (config->ragebot[weaponIndex].visibleOnly && !entity->isVisible(bonePosition))
                        continue;

                    auto damage = canScan(entity, bonePosition, activeWeapon->getWeaponData(),config->ragebot[weaponIndex].minDamage, config->ragebot[weaponIndex].friendlyFire);
                    if (damage <= 0.f)
                        continue;
            ...
            if (record && !record->empty() && record->size() > 3 && Backtrack::valid(record->front().simulationTime) && config->backtrack.enabled)
            {
                ...
                for (int i = 0; i < 19; i++)
                {
                    ...
                    for (auto bonePosition : Multipoint(entity, currentRecord.matrix, i, weaponIndex, firstScan))
                    {
                        ...
                        printf(std::to_string(damage).c_str());
                        printf("\n");
