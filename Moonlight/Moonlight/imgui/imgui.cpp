...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ...
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(ImGuiCol_TextDisabled), text, text_display_end);
        if (g.LogEnabled)
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
    ...
    if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground)) {
        //child borders border
        //window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
        window->DrawList->AddRect(ImVec2(window->Pos.x, window->Pos.y), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), GetColorU32(ImVec4(0.20f, 0.20f, 0.20f, 1.00f)), rounding, ImDrawCornerFlags_All, border_size);
        window->DrawList->AddRect(ImVec2(window->Pos.x + 1, window->Pos.y + 1), ImVec2(window->Pos.x + window->Size.x - 1, window->Pos.y + window->Size.y - 1), GetColorU32(ImVec4(0.18f, 0.18f, 0.18f, 1.00f)), rounding, ImDrawCornerFlags_All, border_size);
    ...
    for (int i = 0; i < val; i++) {
      ...
      //  window->DrawList->AddRect(ImVec2(window->Pos.x - val + i, window->Pos.y - val + i), ImVec2(window->Pos.x + (window->Size.x + val) - i, window->Pos.y + (window->Size.y + val) - i), ImColor(0, 0, 0, i * gui->border_animation));
    }
    ...
    if (g.Style.FrameBorderSize > 0 && !(window->Flags & ImGuiWindowFlags_NoTitleBar))
    {
        ...
        //window->DrawList->AddLine(ImVec2(window->Pos.x + border_size, y), ImVec2(window->Pos.x + window->Size.x - border_size, y), GetColorU32(ImVec4(0.f, 0.f, 0.f, 1.f)), g.Style.FrameBorderSize);
    }
}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    } else
    {
        ...
        if (flags & ImGuiWindowFlags_CustomTopBar) {
            window->DrawList->AddText(g.Font, g.FontSize * 1.0f, ImVec2(title_bar_rect.Min.x + 5, title_bar_rect.Min.y + 3), ImGui::GetColorU32(ImGuiCol_Text), szCheatName);
            window->DrawList->AddLine(ImVec2(title_bar_rect.Min.x, title_bar_rect.Min.y + 20), ImVec2(title_bar_rect.Max.x, title_bar_rect.Max.y + 20), GetColorU32(ImVec4(0.7f, 0.1f, 0.7f, 1.f)), 2);
            window->DrawList->AddLine(ImVec2(title_bar_rect.Min.x, title_bar_rect.Min.y + 21), ImVec2(title_bar_rect.Max.x, title_bar_rect.Max.y + 21), GetColorU32(ImVec4(0.f, 0.f, 0.f, 0.2f)));
        }
