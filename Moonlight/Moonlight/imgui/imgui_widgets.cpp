...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (ImGui::IsItemHovered()) {
        col = ImGui::GetColorU32(ImLerp(ImColor(20, 90, 190, 0), ImVec4(0.2, 0.2, 0.2, 0), t));
    }
    else {
        col = ImGui::GetColorU32(ImLerp(ImColor(0, 0, 0, 0), ImVec4(0.2, 0.2, 0.2, 0), t));
    }
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    window->DrawList->AddRectFilled(ImVec2(bb_frame.Min.x, bb_frame.Min.y), bb_frame.Max, bg_col, window->WindowRounding, rounding_corners);
    ...
    window->DrawList->AddRectFilled(ImVec2(grab_rect.Min.x, grab_rect.Min.y), grab_rect.Max, grab_col);
...
bool ImGui::ToggleButton(const char* str_id, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 1.0f));
    ...
    ImU32 col_bg;
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.30f, 0.30f, 0.30f, 1.0f), ImVec4(0.4f, 0.4f, 0.4f, 1.0f), t));
    else
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.30f, 0.30f, 0.30f, 1.0f), ImVec4(0.4f, 0.4f, 0.4f, 1.0f), t));
    ...
    ImU32 mmm;
    if (ImGui::IsItemHovered())
        mmm = ImGui::GetColorU32(ImLerp(ImVec4(0.20f, 0.20f, 0.20f, 1.0f), ImVec4(0.80f, 0.80f, 0.80f, 1.0f), t));
    else
        mmm = ImGui::GetColorU32(ImLerp(ImVec4(0.20f, 0.20f, 0.20f, 1.0f), ImVec4(0.80f, 0.80f, 0.80f, 1.0f), t));


    draw_list->AddCircleFilled(ImVec2(p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - 2.0f, IM_COL32(0.160f * 255, 0.160f * 255, 0.160f * 255, 1.f * 255));
    draw_list->AddCircleFilled(ImVec2(p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - 5.0f, mmm);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 12, check_bb.Min.y + style.FramePadding.y), str_id);

    return v;
}


bool ImGui::BigCheckbox(const char* str_id, bool* v)
{
    ImVec2 p = ImGui::GetCursorScreenPos();
    ImVec2 ps = ImVec2(p.x + 194, p.y);//194
    ...
    ImGui::InvisibleButton(str_id, ImVec2(ps.x, height));
    if (ImGui::IsItemClicked())
        *v = !*v;
    ...
    ImU32 col_txt;
    if (ImGui::IsItemHovered())
        col_txt = ImGui::GetColorU32(ImLerp(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), ImVec4(0.95f, 0.95f, 0.95f, 0.80f), t));
    else
        col_txt = ImGui::GetColorU32(ImLerp(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), ImVec4(0.95f, 0.95f, 0.95f, 0.80f), t));
    ...
    ImU32 col_bg;
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.3f, 0.3f, 0.3f, 1.0f), ImColor(140, 70, 190, 255), t));
    else
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.3f, 0.3f, 0.3f, 1.0f), ImColor(140, 70, 190, 255), t));

    draw_list->AddText(ImVec2(p.x, p.y), col_txt, str_id);

    //outline
    draw_list->AddRect(ImVec2(ps.x + 1, p.y + 3), ImVec2(ps.x + width - 1, p.y + 11), ImColor(0, 0, 0, 60), height * 0.5f);
    draw_list->AddRectFilled(ImVec2(ps.x + 2, p.y + 4), ImVec2(ps.x + width - 2, p.y + 10), col_bg, height * 0.5f);

    for (int i = 0; i < 5; i++) {
        draw_list->AddCircleFilled(ImVec2(ps.x + radius + t * (width - radius * 2.0f), p.y + radius), 4 + i, ImColor(100, 55, 150, td * 50), 50);
    }

    draw_list->AddCircleFilled(ImVec2(ps.x + radius + t * (width - radius * 2.0f), p.y + radius), 6, ImColor(140, 70, 190, 255), 30);
...
{
    ImVec2 p = ImGui::GetCursorScreenPos();
    ImVec2 ps = ImVec2(p.x - 300, p.y);
    ImDrawList* draw_list = ImGui::GetWindowDrawList();

    float height = 15;
    float width = height * 1.00f;
    float radius = height * 0.50f;
    const ImRect check_bb(p, p + ImVec2(width, height));
    float t = *v ? 1.0f : 0.0f;
    ImGuiContext& g = *GImGui;
    float ANIM_SPEED = 0.25f;

    ImGui::InvisibleButton(label, ImVec2(90, height));
    if (ImGui::IsItemClicked())
        *v = !*v;
    ...
    double td = t;
    ImU32 col_txt;
    if (ImGui::IsItemHovered()) col_txt = ImGui::GetColorU32(ImLerp(ImVec4(0.5f, 0.5f, 0.5f, gui->menu_animation), ImColor(95, 75, 113, gui->menu_animation), t));
    else col_txt = ImGui::GetColorU32(ImLerp(ImVec4(0.5f, 0.5f, 0.5f, gui->menu_animation), ImColor(95, 75, 113, gui->menu_animation), t));

    ImU32 col_bg;
    if (ImGui::IsItemHovered()) col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.3f, 0.3f, 0.3f, 1.0f), ImColor(140, 70, 190, 255), t));
    else col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.3f, 0.3f, 0.3f, 1.0f), ImColor(140, 70, 190, 255), t));

    float tan = *v ? 1.0f : 0.0f;
    if (g.LastActiveId == g.CurrentWindow->GetID(label)) {
        float t_animd = ImSaturate(g.LastActiveIdTimer / 0.35);
        tan = *v ? (t_animd) : (1.0f - t_animd);
    }

    ImU32 col_check;
    if (ImGui::IsItemHovered()) col_check = ImGui::GetColorU32(ImLerp(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.0f), ImVec4(30 / 255.f, 30 / 255.f, 30 / 255.f, 1.0f), tan));
    else col_check = ImGui::GetColorU32(ImLerp(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.0f), ImVec4(30 / 255.f, 30 / 255.f, 30 / 255.f, 1.0f), tan));

    draw_list->AddText(ImVec2(p.x, p.y), col_txt, label);

    return v;
}


bool ImGui::ButtonExT(bool sub_tab, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddLine(ImVec2(pos.x - 7, pos.y - 5), ImVec2(pos.x + 97, pos.y - 5), ImColor(0, 0, 0, 100), 1.f);

    if (page == in) {

        if (!sub_tab) {

            window->DrawList->AddRectFilledMultiColor(ImVec2(pos.x - 8, pos.y - 4), ImVec2(pos.x + 97, pos.y + 23), ImColor(0, 0, 0, 150), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 150));
        }

        PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 0.6f));
...
//custom button
bool ImGui::ButtonT(bool sub_tab, const char* label, const ImVec2& size_arg, int page, int in, ImColor clr, bool border) {
    return ButtonExT(sub_tab, label, size_arg, 0, page, in, border);
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f, label_size.x > 0.0f ? label_size.y : 0.0f), window->DC.CursorPos + ImVec2(w, (label_size.x > 0.0f ? label_size.y : 0.0f) + h - 3));
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_WindowBg), true, g.Style.FrameRounding);
	//PopStyleColor();
...
bool ImGui::TabItemButton(const char* label, ImGuiTabItemFlags flags)
{
    ...
    return TabItemEx(tab_bar, label, NULL, flags);
}
